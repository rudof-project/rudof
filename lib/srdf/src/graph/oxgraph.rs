...
impl<T: Triple> Rdf for GenericGraph<T> {
    ...
    fn triples_matching(
        ...
        let (subject, predicate, object) = (subject.into(), predicate.into(), object.into());

        let triples = self.graph.iter().filter_map(move |triple| {
            let subj: Matcher<Self> = triple.subject().clone().into();
            let pred: Matcher<Self> = triple.predicate().clone().into();
            let obj: Matcher<Self> = triple.object().clone().into();

            if subject == subj && predicate == pred && object == obj {
                let (s, p, o) = triple.spo();
                Some(Self::Triple::from_spo(s.clone(), p.clone(), o.clone()))
            } else {
                None
            }
        });
...
impl RdfReader for GenericGraph<OxTriple> {
    ...
    fn merge_from_reader<R: Read>(
        ...
        for triple in reader {
            ...
            self.add_triple_from_components(triple.subject, triple.predicate, triple.object)?;
        }
...
mod tests {
    ...
    fn test_outgoing_arcs() {
        ...
        let subject = graph
            .triples_matching(x, p.clone(), Matcher::Any)
    ...
    fn test_add_triple() {
        ...
        graph.add_triple_from_components(alice, knows, bob).unwrap();
