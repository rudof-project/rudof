...
impl<R: Rdf, E: Engine<R>> NativeValidator<R, E> for Datatype<R> {
    fn validate_native(
        &self,
        ...
        let datatype = |value_node: &Object<R>| {
            if let Some(literal) = value_node.literal() {
                return Predicate::<R>::from_str(literal.datatype()) != *self.datatype();
            }
            true
        };
...
impl<S: Rdf + Sparql> SparqlValidator<S> for Datatype<S> {
    fn validate_sparql(
        &self,
