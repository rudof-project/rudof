...
impl<R: FocusRdf + PrefixMapRdf + Default + Clone> ShaclParser<R> {
    ...
    fn shapes_candidates(&mut self) -> Result<Vec<Object<R>>> {
        ...
        let node_shape_instances = self
            .rdf_parser
            .rdf
            .triples_matching(Matcher::Any, Self::rdf_type(), Self::sh_node_shape())
...
fn property_shape<'a, R: FocusRdf>(
    ...
    optional(has_type(&SH_PROPERTY_SHAPE))
        .with(
            id().and(path())
                .then(|(id, path)| ok(PropertyShape::new(id, path))),
        )
        .then(|ps| targets().flat_map(move |ts| Ok(ps.with_targets(ts))))
        .then(|ps| {
            optional(closed()).flat_map(move |c| match c {
                Some(true) => Ok(ps.with_closed(true)),
                _ => Ok(ps),
            })
        })
        .then(|ps| property_shapes().flat_map(move |s| Ok(ps.with_property_shapes(s))))
...
fn node_shape<R: FocusRdf>() -> impl RDFNodeParse<R, Output = NodeShape<R>> {
    not(property_values_non_empty(&SH_PATH)).with(
        term()
            ...
            .then(|ns| targets().flat_map(move |ts| Ok(ns.with_targets(ts))))
            .then(|ps| {
                optional(closed()).flat_map(move |c| match c {
                    Some(true) => Ok(ps.with_closed(true)),
                    _ => Ok(ps),
            ...
            .then(|ns| property_shapes().flat_map(move |ps| Ok(ns.with_property_shapes(ps))))
            .then(|ns| components().flat_map(move |cs| Ok(ns.with_components(cs)))),
    )
}
