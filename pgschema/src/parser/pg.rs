...
impl std::fmt::Debug for ProdKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let name = match self {
            ...
            ProdKind::RecordP1 => "Record: OPENSQUAREBRACKET Properties CLOSESQUAREBRACKET",
...
fn action_bool_s52(token_kind: TokenKind) -> Vec<Action<State, ProdKind>> {
    match token_kind {
        ...
        TK::CLOSESQUAREBRACKET => Vec::from(&[Reduce(PK::SingleValueBooleanValue, 1usize)]),
        _ => vec![],
    }
}
...
pub struct PgParser<'i, I: InputT + ?Sized, L: Lexer<'i, Context<'i, I>, State, TokenKind, Input = I>, B>(
    LRParser<'i, Context<'i, I>, State, ProdKind, TokenKind, NonTermKind, PgParserDefinition, L, B, I>,
);
#[allow(dead_code)]
impl<'i>
    PgParser<
...
impl<'i> LRBuilder<'i, Input, Context<'i, Input>, State, ProdKind, TokenKind> for DefaultBuilder {
    ...
    fn shift_action(&mut self, context: &Context<'i, Input>, token: Token<'i, Input, TokenKind>) {
        let val = match token.kind {
            ...
            TokenKind::QUOTED_STRING => Terminal::QUOTED_STRING(pg_actions::quoted_string(context, token)),
        ...
        let prod = match prod {
            ...
            ProdKind::StatementsP1 => {
                ...
                match i.next().unwrap() {
                    Symbol::NonTerminal(NonTerminal::Statement1(p0)) => {
                        NonTerminal::Statements(pg_actions::statements_statement1(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::Statement1P1 => {
                ...
                match (i.next().unwrap(), i.next().unwrap()) {
                    ...
                    ) => NonTerminal::Statement1(pg_actions::statement1_c1(context, p0, p1)),
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::Statement1P2 => {
                ...
                match i.next().unwrap() {
                    Symbol::NonTerminal(NonTerminal::Statement(p0)) => {
                        NonTerminal::Statement1(pg_actions::statement1_statement(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ...
            ProdKind::EdgeP1 => {
                let mut i = self.res_stack.split_off(self.res_stack.len() - 12usize).into_iter();
                match (
                    i.next().unwrap(),
                    i.next().unwrap(),
                    i.next().unwrap(),
            ...
            ProdKind::EdgeIdOptP1 => {
                ...
                match i.next().unwrap() {
                    Symbol::NonTerminal(NonTerminal::EdgeId(p0)) => {
                        NonTerminal::EdgeIdOpt(pg_actions::edge_id_opt_edge_id(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::EdgeIdOptP2 => NonTerminal::EdgeIdOpt(pg_actions::edge_id_opt_empty(context)),
            ProdKind::NodeIdP1 => {
                let mut i = self.res_stack.split_off(self.res_stack.len() - 1usize).into_iter();
                match i.next().unwrap() {
            ...
            ProdKind::LabelsRecordP1 => {
                ...
                match (i.next().unwrap(), i.next().unwrap()) {
                    ...
                    ) => NonTerminal::LabelsRecord(pg_actions::labels_record_c1(context, p0, p1)),
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::LabelsOptP1 => {
                ...
                match i.next().unwrap() {
                    Symbol::NonTerminal(NonTerminal::Labels(p0)) => {
                        NonTerminal::LabelsOpt(pg_actions::labels_opt_labels(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::LabelsOptP2 => NonTerminal::LabelsOpt(pg_actions::labels_opt_empty(context)),
            ProdKind::RecordOptP1 => {
                let mut i = self.res_stack.split_off(self.res_stack.len() - 1usize).into_iter();
                match i.next().unwrap() {
                    Symbol::NonTerminal(NonTerminal::Record(p0)) => {
                        NonTerminal::RecordOpt(pg_actions::record_opt_record(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::RecordOptP2 => NonTerminal::RecordOpt(pg_actions::record_opt_empty(context)),
            ProdKind::LabelsP1 => {
            ...
            ProdKind::PropertyP1 => {
                let mut i = self.res_stack.split_off(self.res_stack.len() - 3usize).into_iter();
                match (i.next().unwrap(), i.next().unwrap(), i.next().unwrap()) {
                    (Symbol::NonTerminal(NonTerminal::key(p0)), _, Symbol::NonTerminal(NonTerminal::Values(p1))) => {
            ...
            ProdKind::SingleValue1P2 => {
                ...
                match i.next().unwrap() {
                    Symbol::NonTerminal(NonTerminal::SingleValue(p0)) => {
                        NonTerminal::SingleValue1(pg_actions::single_value1_single_value(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::SingleValue0P1 => {
                ...
                match i.next().unwrap() {
                    Symbol::NonTerminal(NonTerminal::SingleValue1(p0)) => {
                        NonTerminal::SingleValue0(pg_actions::single_value0_single_value1(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::SingleValue0P2 => NonTerminal::SingleValue0(pg_actions::single_value0_empty(context)),
            ...
            ProdKind::SingleValueDateValue => {
                ...
                match (i.next().unwrap(), i.next().unwrap()) {
                    (_, Symbol::Terminal(Terminal::QUOTED_STRING(p0))) => {
                        NonTerminal::SingleValue(pg_actions::single_value_date_value(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::SingleValueBooleanValue => {
                ...
                match i.next().unwrap() {
                    Symbol::NonTerminal(NonTerminal::BOOL(p0)) => {
                        NonTerminal::SingleValue(pg_actions::single_value_boolean_value(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ...
            ProdKind::identifierP1 => {
                ...
                match i.next().unwrap() {
                    Symbol::Terminal(Terminal::IDENTIFIER(p0)) => {
                        NonTerminal::identifier(pg_actions::identifier_identifier(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
            ProdKind::identifierP2 => {
                ...
                match i.next().unwrap() {
                    Symbol::Terminal(Terminal::QUOTED_STRING(p0)) => {
                        NonTerminal::identifier(pg_actions::identifier_quoted_string(context, p0))
                    },
                    _ => panic!("Invalid symbol parse stack data."),
                }
            },
