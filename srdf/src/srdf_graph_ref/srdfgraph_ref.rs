...
use crate::srdfgraph_ref_error::SRDFGraphRefError;
use oxrdf::{
    BlankNodeRef as OxBlankNode, Graph, GraphName, LiteralRef as OxLiteral,
    NamedNodeRef as OxNamedNode, NamedNodeRef, Quad, SubjectRef as OxSubject, TermRef as OxTerm,
    TripleRef as OxTriple,
};
use oxttl::{NQuadsParser, NTriplesParser, TurtleParser};
use prefixmap::{prefixmap::*, PrefixMapError};

#[derive(Debug, Default, Clone)]
pub struct SRDFGraphRef<'a> {
    focus: Option<OxTerm<'a>>,
    graph: Graph,
    pm: PrefixMap,
    base: Option<IriS>,
    bnode_counter: usize,
...
impl<'a> SRDFGraphRef<'a> {
                    ...
                    match triple_result {
                        Err(e) => {
                            if reader_mode.is_strict() {
                                return Err(SRDFGraphRefError::TurtleError {
    ...
    pub fn merge_prefixes(&mut self, prefixmap: PrefixMap) -> Result<(), SRDFGraphRefError> {
    ...
    pub fn from_reader<R: io::Read>(
        ...
        base: Option<&str>,
        reader_mode: &ReaderMode,
    ) -> Result<SRDFGraphRef<'a>, SRDFGraphRefError> {
        let mut srdf_graph = SRDFGraphRef::new();

        srdf_graph.merge_from_reader(read, format, base, reader_mode)?;
        Ok(srdf_graph)
    }

    pub fn resolve(&self, str: &'a str) -> Result<OxNamedNode<'a>, SRDFGraphRefError> {
    ...
    fn cnv_iri(iri: &'a IriS) -> OxNamedNode<'a> {
        OxNamedNode::new_unchecked(iri.as_str())
    }

    pub fn add_triple_ref<S, P, O>(
        &mut self,
        subj: S,
        pred: P,
        obj: O,
    ...
    where
        ...
        O: Into<OxTerm<'a>>,
    {
        let subj: OxSubject<'a> = subj.into();
        let pred: OxNamedNode<'a> = pred.into();
        let obj: OxTerm<'a> = obj.into();
        let triple = OxTriple::new(subj, pred, obj);
    ...
    pub fn merge_from_path<P: AsRef<Path>>(
        ...
        let file = File::open(path.as_ref()).map_err(|e| SRDFGraphRefError::ReadingPathError {
...
impl<'a> Rdf for SRDFGraphRef<'a> {
    type IRI = OxNamedNode<'a>;
    type BNode = OxBlankNode<'a>;
    type Literal = OxLiteral<'a>;
    type Subject = OxSubject<'a>;
    type Term = OxTerm<'a>;
    type Triple = OxTriple<'a>;
    type Err = SRDFGraphRefError;
...
impl<'a> SRDFBuilder for SRDFGraphRef<'a> {
    ...
    fn add_bnode(&mut self) -> Result<Self::BNode, Self::Err> {
        ...
        match self.bnode_counter.try_into() {
            ...
            Err(_) => Err(SRDFGraphRefError::BlankNodeId {
