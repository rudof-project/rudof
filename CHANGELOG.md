...
- `rudof_mcp`: two new tools. `get_rudof_version` reports the rudof version the MCP server is running (matching `rudof --version`). `manage_prefixes` brings the shell's `prefixes` command to MCP sessions — with no `action` (or `action: "list"`) it shows the session's current default prefixes, and `action: "add"/"remove"/"rename"/"copy"` manage them, always returning the full updated list.
- `shex --compiled-schema <FILE>`: loads a precompiled `SchemaIR` cache produced by `--compile-to` (previously only `shex-validate` could load one), to inspect it without the original schema source. Only `-r internal` (plus `--statistics`/`--show-dependencies`) can be used with it — other result formats need the parsed schema (AST), which the cache doesn't preserve, and now fail with an error explaining that instead of a generic "no schema loaded". See the [precompiled ShEx schemas guide](docs/src/using-rudof/precompiled-shex-schemas.md).
- New `ShExFormat`/`--format`/`--schema-format`/`--result-format` value `binary`, unifying the precompiled-cache flags (`--compile-to`/`--compiled-schema`) with `shex`'s and `shex-validate`'s regular `-s`/`-f`/`-r`/`-o` vocabulary: `shex -s schema.shex -r binary -o schema.bin` compiles and writes a cache exactly like `--compile-to`, and `-f binary -s schema.bin` (on either command) loads one back exactly like `--compiled-schema` — both produce/read byte-identical caches. The dedicated flags are unchanged and still work; in both commands they're now implemented as thin shortcuts over the same `ShExFormat::Binary` handling `-f`/`-r binary` use, instead of separate code paths.
- `shacl`: the `sh:message` of a SPARQL-based constraint (`sh:sparql`) can now contain `{?varName}` and `{$varName}` blocks ([SHACL 1.0 §5.3.2](https://www.w3.org/TR/shacl/#results-message), [SHACL 1.2](https://www.w3.org/TR/shacl12-sparql/#sparql-constraints-variables)). The validator replaces each block with the value of that SELECT result variable in `sh:resultMessage` (literals give their lexical form, IRIs give the full IRI). If a variable has no value, the block stays unchanged. The replacement uses the new generic functions `shacl::types::interpolate_message`, `shacl::types::object_to_message_value` and `MessageMap::interpolate`, which take a resolver function. Thus, other result message sources can use them too.
- `shex -r binary -o FILE` (and `--compile-to FILE`) now prints "`N` shape(s) saved in `FILE`" to stderr, so compiling a schema to a cache no longer produces silent output. Kept off `ctx.writer`/stdout deliberately: for `-r binary`, that stream carries the binary cache payload itself (to `FILE` via `-o`, or to stdout without it), and mixing text into it would corrupt the cache.
### Fixed
- `convert -m dctap -x shex`: a DCTAP row with exactly one `valueDataType` (the common case) got a bare triple constraint (`<pred> .`) instead of `<pred> xsd:sometype` — `parse_node_constraint`'s single-datatype branch built the `NodeConstraint` but never wrote it back into the shape expression, silently discarding it. Rows with zero or two-or-more `valueDataType` values were unaffected. Fixed in `shapes_converter`'s DCTAP-to-ShEx converter.
- `shex --no-show-schema`: silently had no effect (the schema was always shown regardless), because clap's `SetFalse` action gives a `bool` field an implicit default of `true`, so `no_show_schema` itself defaulted to `true` even when the flag wasn't passed, and the command code never actually read it anyway. Switched to `SetTrue` with an explicit `default_value_t = false`, matching `--show-schema`'s own declaration, so `overrides_with` correctly makes whichever of `--show-schema`/`--no-show-schema` comes last win; the command now also actually combines both fields. This particularly matters for `-f binary`, where the default `shexc` result format can't be rendered (see the `PrecompiledSchemaFormatUnavailable` fix below) — `--no-show-schema` is now the way to load a precompiled schema (e.g. just to check statistics/dependencies) without that error.
- `shex`/`shex-validate` internal: `serialize_shex_schema`'s `-r internal` (and `--statistics`/`--show-dependencies`) unconditionally required the parsed schema (AST) even though they only read the compiled `SchemaIR`, so a schema loaded via `load_shex_schema_precompiled` (which only sets the `SchemaIR`, not the AST) always failed with "No ShEx schema loaded" for those formats too, not just AST-dependent ones like `shexc`/`shexj`. The AST is now fetched lazily, only for the formats that actually need it.
- `query -e <endpoint>` against SPARQL endpoints that append non-standard top-level fields to their JSON results (e.g. QLever's `"meta":{"query-time-ms":...,"result-size-total":...}` after `results`) failed with "Error parsing solution: Unexpected JSON after the end of the bindings array", since the strict `sparesults` reader used to parse SELECT/ASK results only tolerates `head`/`results`/`boolean` at the top level. The response body is now sanitized (unrecognized top-level keys stripped) before parsing, so such endpoints (e.g. `wikidata_qlever`) work like any other.
...
- ShExJ: numeric values (`xsd:integer`, `xsd:decimal`, `xsd:double`) in value sets and annotations now keep their lexical form. The ShExC parser and the RDF data do the same. Before this fix, a numeric value set from a ShExJ schema did not match numeric values in the data, and the ShExJ to ShExC round trip failed for these values. The compact printer now writes these values as bare numbers (for example `[ 0 ]`) when the parser reads them back as the same literal. The ShExJ to ShExC round trip now passes for 13 more entries of the shexTest suite, for example `1val1INTEGER`.
...
- `--qlever-index-dir <DIR>`: new CLI flag (any command that accepts `--backend`) to pin the QLever on-disk index to a specific directory, overriding `[rdf.qlever] index_dir` without needing a TOML config file. Lets a user share a built index between rudof sessions/machines by pointing them at the same directory — the (idempotent) index-build step is skipped whenever it's already there. The resolved index directory (explicit or auto-fingerprinted under the platform cache dir) is now also logged at startup so it's always visible (#804)
### Fixed
### Changed
### Removed
