...
impl Command for NodeCommand {
    ...
    fn execute(&self, ctx: &mut CommandContext) -> Result<()> {
        ...
        let mut showing_node_info = ctx.rudof.show_node_info(self.args.node.as_ref(), &mut ctx.writer)
            .with_show_node_mode(&show_node_mode)
            .with_depth(self.args.depth);
        if let Some(predicates) = self.args.predicates.as_deref() { showing_node_info = showing_node_info.with_predicates(predicates); }
        if let Some(show_hyperlinks) = self.args.show_hyperlinks { showing_node_info = showing_node_info.with_show_hyperlinks(show_hyperlinks); }
