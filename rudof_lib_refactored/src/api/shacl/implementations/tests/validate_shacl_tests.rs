use crate::{
    ...
    formats::{InputSpec, ShaclFormat, DataFormat, ResultShaclValidationFormat, ShaclValidationMode, ShaclValidationSortByMode},
};
use std::str::FromStr;
...
fn test_serialize_validation_results_without_validation() {
    ...
    let mut buffer = Vec::new();
    let result = serialize_shacl_validation_results(
        &rudof,
        None,
        Some(&ResultShaclValidationFormat::Minimal),
        &mut buffer,
    );
