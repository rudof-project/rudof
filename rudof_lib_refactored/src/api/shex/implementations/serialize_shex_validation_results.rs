...
pub fn serialize_shex_validation_results<W: io::Write>(
    ...
    let shex_validation_results = rudof
        ...
        .as_ref()
        .ok_or_else(|| ShExError::NoShexValidationResultsAvailable)?;
    ...
    match result_shex_validation_format {
        ResultShExValidationFormat::Compact => {
            ...
            shex_validation_results
                ...
                .map_err(|e| ShExError::FailedSerializingShExValidationResults { format: "compact".to_string(), error: e.to_string() })?;
        },
        ResultShExValidationFormat::Csv => {
            shex_validation_results
                .as_csv(writer, sort_order.into(), true)
                .map_err(|e| ShExError::FailedSerializingShExValidationResults { format: "csv".to_string(), error: e.to_string() })?;
        },
        ResultShExValidationFormat::Details => {
            writeln!(writer, "Results:").map_err(|e| ShExError::FailedIoOperation { error: e.to_string() })?;

            shex_validation_results.as_table(writer, Some(&sort_order.into()), Some(true), Some(terminal_width()))
                .map_err(|e| ShExError::FailedSerializingShExValidationResults { format: "details".to_string(), error: e.to_string() })?;
        },
        ResultShExValidationFormat::Json => {
            writeln!(writer, "Results:").map_err(|e| ShExError::FailedIoOperation { error: e.to_string() })?;

            let str = serde_json::to_string_pretty(&shex_validation_results)
                .map_err(|e| ShExError::FailedSerializingShExValidationResults { format: "json".to_string(), error: e.to_string() })?;
