use crate::{
    ...
    formats::{InputSpec, ShExFormat, ShapeMapFormat, DataFormat},
};
use std::str::FromStr;
...
fn test_load_shapemap_replace() {
    ...
    // Load first shapemap
    let shapemap1 = InputSpec::from_str(
        r#"ex:alice@ex:PersonShape"#,
    )
    .unwrap();
    ...
    let shapemap2 = InputSpec::from_str(
        r#"ex:bob@ex:EmployeeShape"#,
    )
    .unwrap();
...
fn test_load_shapemap_invalid_syntax() {
    ...
    let result = load_shapemap(&mut rudof, &invalid_shapemap, Some(&ShapeMapFormat::Compact), None, None);
...
fn test_load_shapemap_with_base_nodes() {
    ...
    // Load data
    let data = InputSpec::from_str(
        r#"<alice> <http://example.org/name> "Alice" ."#,
    )
    .unwrap();
