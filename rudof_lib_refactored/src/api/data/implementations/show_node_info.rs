...
pub fn show_node_info<W: io::Write>(
    ...
    let config = NodeDisplayConfig::from_options(
        predicates,
        show_node_mode,
        depth,
        show_hyperlinks,
    ...
    let data = rudof
        .data
        .as_mut()
        .ok_or(DataError::NoRdfDataLoaded)?;

    if !data.is_rdf() {
        return Err(DataError::NoRdfDataLoaded.into());
    }

    let node_selector = parse_node_selector(node)?;

    let node_infos = collect_node_information(
        data.unwrap_rdf_mut(),
        node_selector,
        &config,
    )?;

    write_node_information_list(
        &node_infos,
        data.unwrap_rdf_mut(),
        writer,
        &config,
    ...
    R: NeighsRDF + Debug + QueryRDF,
{
    let nodes = node_selector
        .nodes(rdf)
        .map_err(|e| DataError::FailedArcRetrieval{
            error: e.to_string(),
        })?;

    nodes
        .iter()
        .map(|node| build_node_info(rdf, node, config))
        .collect()
}
...
where
    R: NeighsRDF + Debug + QueryRDF,
{
    let subject = R::term_as_subject(node)
        .map_err(|e| DataError::FailedQualification {
            error: e.to_string(),
        })?;
    ...
    let outgoing = if config.mode.show_outgoing() {
        collect_outgoing_arcs_recursive(
            rdf,
            &subject,
            &config.predicates,
            config.depth,
...
fn write_node_information<S: NeighsRDF, W: io::Write>(
    ...
    if config.mode.show_outgoing() && !node_info.outgoing.is_empty() {
        writeln!(writer, "Outgoing arcs")
            .map_err(|e| DataError::FailedIoOperation {
                error: e.to_string(),
            })?;
        
        let mut tree = Tree::new(node_info.subject_qualified.clone())
            .with_glyphs(create_outgoing_glyphs());
    ...
    if config.mode.show_incoming() && !node_info.incoming.is_empty() {
        ...
        let mut tree = Tree::new(root_label)
            .with_glyphs(create_incoming_glyphs());
            ...
            for object_node in objects {
                match object_node {
                    OutgoingNeighsNode::Term { term } => {
                        ...
                        tree.leaves.push(
                            Tree::new(label).with_glyphs(create_outgoing_glyphs())
                        );
                    }
                    OutgoingNeighsNode::More { term, rest } => {
                        ...
                        let mut subtree = Tree::new(label)
                            .with_glyphs(create_outgoing_glyphs());
            ...
            for subject_node in subjects {
                match subject_node {
                    IncomingNeighsNode::Term { term } => {
                        ...
                        tree.leaves.push(
                            Tree::new(label).with_glyphs(create_incoming_glyphs())
                        );
                    }
            ...
            let iri_s = match iri_ref {
                IriRef::Prefixed { prefix, local } => {
                    ...
                    rdf.resolve_prefix_local(prefix.as_str(), local.as_str())
                        .map_err(|e| DataError::FailedPrefixResolution {
...
fn qualify_term<S: NeighsRDF>(
    ...
    if show_colors {
        ...
        let node = S::term_as_object(term)
            .map_err(|e| DataError::FailedQualification {
                error: e.to_string(),
            })?;
