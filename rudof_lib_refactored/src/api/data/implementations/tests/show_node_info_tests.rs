use crate::{
    ...
    formats::NodeInspectionMode, errors::{DataError, RudofError},
};
...
fn setup_test_rudof() -> Rudof {
    ...
    rudof.load_data().with_data(&vec![rdf_data]).execute().expect("Failed to load test RDF data");
...
fn setup_deep_graph_rudof() -> Rudof {
    ...
    rudof.load_data().with_data(&vec![rdf_data]).execute().expect("Failed to load deep graph RDF data");
...
#[test]
fn test_show_node_info_invalid_node_selector() {
    let mut rudof = setup_test_rudof();
    let mut output = Cursor::new(Vec::new());
    ...
    match result {
        Err(RudofError::Data(DataError::FailedNodeSelectorParse { .. })) => {
        }
...
#[test]
fn test_show_node_info_no_data_loaded() {
    let mut rudof = Rudof::new(RudofConfig::default());
    let mut output = Cursor::new(Vec::new());
    ...
    match result {
        Err(RudofError::Data(DataError::NoRdfDataLoaded)) => {
            
        }
        Err(e) => panic!("Unexpected error: {:?}", e),
        Ok(_) => panic!("Expected NoRdfDataLoaded error"),
    }
}
