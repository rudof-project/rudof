...
pub fn show_schema_conversion<W: io::Write>(
    ...
    match (input_mode, output_mode) {
        ...
        (ConversionMode::Dctap, ResultConversionMode::Uml) => show_schema_conversion_dctap_to_uml(
            rudof,
            schema,
            input_format,
            output_format,
        ...
        (ConversionMode::Dctap, ResultConversionMode::Html) => match output_folder {
            Some(of) => show_schema_conversion_dctap_to_html(
                rudof,
                schema,
                input_format,
                output_format,
...
fn show_schema_conversion_shex_to_shex<W: io::Write>(
    ...
    load_shex_schema(
        rudof,
        schema,
        Some(&input_format.clone().try_into()?),
        base,
        reader_mode,
    )?;
    ...
    serialize_shex_schema(
        ...
        show_time,
        None,
        Some(&output_format.clone().try_into()?),
        writer,
    )?;
...
fn show_schema_conversion_shex_to_sparql<W: io::Write>(
    ...
    let shape = match &shape {
        ...
        Some(shape_str) => {
            let iri_shape =
                ShapeMapParser::parse_iri_ref(shape_str).map_err(|error| ConversionError::ConversionError {
                    input_mode: "shex".to_string(),
    ...
    let sparql = converter
        ...
        .map_err(|error| ConversionError::ConversionError {
            input_mode: "shex".to_string(),
            output_mode: "sparql".to_string(),
            input_format: input_format.to_string(),
            output_format: output_format.to_string(),
    ...
    write!(writer, "{sparql}").map_err(|error| ConversionError::ConversionError {
        input_mode: "shex".to_string(),
        output_mode: "sparql".to_string(),
        input_format: input_format.to_string(),
        output_format: output_format.to_string(),
...
fn show_schema_conversion_shacl_to_shacl<W: io::Write>(
    ...
    serialize_shacl_schema(rudof, Some(&output_format.clone().try_into()?), writer)?;
...
fn show_schema_conversion_dctap_to_shex<W: io::Write>(
    ...
    load_dctap(rudof, schema, Some(&input_format.clone().try_into()?))?;

    let converter = Tap2ShEx::new(&rudof.config.tap2shex_config());
    let shex_schema = converter
