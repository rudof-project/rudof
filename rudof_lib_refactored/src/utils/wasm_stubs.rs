...
impl ClientBuilder {
    ...
    pub fn build(&self) -> Result<Client, WASMError> {
        Ok(Client)
    }
...
impl Client {
    ...
    ///
    /// # Errors
    /// Always returns a WASM error indicating that HTTP client operations
    /// are not implemented in WASM environments.
    pub fn send(self) -> Result<Response, WASMError> {
        Err(WASMError::HttpClientNotSupported)
    }
}
...
impl Read for Response {
    ...
    ///
    /// # Errors
    /// Always returns an error indicating that HTTP response reading
    /// is not implemented in WASM environments.
    fn read(&mut self, _buf: &mut [u8]) -> std::io::Result<usize> {
        Err(IoError::other(WASMError::HttpResponseNotSupported))
    }
}
