...
use crate::{
    Rudof, RudofConfig, formats::{InputSpec, NodeInspectionMode}, types::NeighborArc,
};
use rudof_iri::IriS;
...
fn test_node_neighborhood_incoming_outgoing_depth_1() {
    ...
    let mut neighborhood = rudof.node_neighborhood("<http://example.org/a>").with_mode(&NodeInspectionMode::Both).execute().unwrap();
