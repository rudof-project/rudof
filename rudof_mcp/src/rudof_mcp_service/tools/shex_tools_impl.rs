...
use rudof_lib::{
    InputSpec, RudofConfig, shex::parse_shex_schema,
    shex::serialize_current_shex_rudof, shex::serialize_shape_current_shex_rudof,
    shex_format::ShExFormat, parse_shape_selector,
};
...
pub async fn show_shex_impl(
    ...
    let parsed_schema_format: ShExFormat = match schema_format {
        Some(s) => ShExFormat::from_str(&s).map_err(|e| {
            invalid_request_error("Invalid schema format", e.to_string(),
    ...
    let parsed_result_format: ShExFormat = match result_schema_format {
        Some(s) => ShExFormat::from_str(&s).map_err(|e| {
            invalid_request_error("Invalid result schema format", e.to_string(),
    ...
    let parsed_base_schema: Option<IriS> = match base_schema {
        Some(s) => Some(IriS::from_str(&s).map_err(|e| {
            invalid_request_error("Invalid base IRI", e.to_string(),
    ...
    let parsed_reader_mode: ReaderMode = match reader_mode {
        Some(s) => ReaderMode::from_str(&s).map_err(|e| {
            invalid_request_error("Invalid reader mode", e.to_string(),
    ...
    parse_shex_schema(&mut rudof, &schema_spec, &parsed_schema_format, &parsed_base_schema, &parsed_reader_mode, &rudof_config)
        .map_err(|e| internal_error("Parsing ShEx schema failed", e.to_string(), Some(json!({"operation":"show_shex_impl","phase":"parse_shex"}))))?;
    ...
    if let Some(shape_selector_str) = shape {
        let shape_selector = parse_shape_selector(&shape_selector_str).map_err(|e| {
            invalid_request_error("Invalid shape selector", e.to_string(), Some(json!({"operation":"show_shex_impl","phase":"parse_shape"})))
        })?;
        let formatter = shex_ast::compact::ShExFormatter::default().without_colors();
        serialize_shape_current_shex_rudof(&rudof, &shape_selector, &parsed_result_format, &formatter, &mut output_buffer)
            .map_err(|e| internal_error("Serialization failed", e.to_string(), Some(json!({"operation":"show_shex_impl","phase":"serialize_shape"}))))?;
    } else if show_schema.unwrap_or(true) {
        let formatter = shex_ast::compact::ShExFormatter::default().without_colors();
        serialize_current_shex_rudof(&rudof, &parsed_result_format, &formatter, &mut output_buffer)
            .map_err(|e| internal_error("Serialization failed", e.to_string(), Some(json!({"operation":"show_shex_impl","phase":"serialize_schema"}))))?;
    }
    ...
    let output_str = String::from_utf8(output_bytes).map_err(|e| {
        internal_error("Conversion error", e.to_string(), Some(json!({"operation":"show_shex_impl","phase":"utf8_conversion"})))
    })?;
    ...
    let structured = serde_json::to_value(&response).map_err(|e| {
        internal_error("Serialization error", e.to_string(), Some(json!({"operation":"show_shex_impl","phase":"serialize_response"})))
    })?;
...
pub async fn check_shex_impl(
    ...
    Parameters(CheckShexRequest { schema, schema_format, base_schema, reader_mode }): Parameters<CheckShexRequest>,
) -> Result<CallToolResult, McpError> {
    let parsed_schema_format: ShExFormat = match schema_format {
        Some(s) => ShExFormat::from_str(&s).map_err(|e| {
            invalid_request_error("Invalid schema format", e.to_string(),
    ...
    match parse_shex_schema(&mut tmp, &schema_spec, &parsed_schema_format, &parsed_base_schema, &parsed_reader_mode, &rudof_config) {
        Ok(_) => {
            let elapsed = begin.elapsed().as_secs_f64();
            let response = CheckShexResponse { valid: true, message: None, elapsed_seconds: Some(elapsed) };
            let structured = serde_json::to_value(&response).map_err(|e| {
                internal_error("Serialization error", e.to_string(), Some(json!({"operation":"check_shex_impl","phase":"serialize_response"})))
            })?;
        ...
        Err(err) => {
            ...
            let elapsed = begin.elapsed().as_secs_f64();
            let response = CheckShexResponse { valid: false, message: Some(err.to_string()), elapsed_seconds: Some(elapsed) };
            let structured = serde_json::to_value(&response).map_err(|e| {
                internal_error("Serialization error", e.to_string(), Some(json!({"operation":"check_shex_impl","phase":"serialize_response_error"})))
            })?;
...
pub async fn shape_info_impl(
    service: &RudofMcpService,
    Parameters(ShapeInfoRequest { schema, schema_format, base_schema, reader_mode, shape, result_schema_format, compile, show_time, show_dependencies, show_statistics }): Parameters<ShapeInfoRequest>,
) -> Result<CallToolResult, McpError> {
    let parsed_schema_format: ShExFormat = match schema_format {
        Some(s) => ShExFormat::from_str(&s).map_err(|e| {
            invalid_request_error("Invalid schema format", e.to_string(),
    ...
    let schema_spec = InputSpec::Str(schema.clone());
    parse_shex_schema(&mut rudof, &schema_spec, &parsed_schema_format, &parsed_base_schema, &parsed_reader_mode, &rudof_config)
        .map_err(|e| internal_error("Parsing ShEx schema failed", e.to_string(), Some(json!({"operation":"shape_info_impl","phase":"parse_shex"}))))?;

    // Parse shape selector and serialize only that shape
    let shape_selector = parse_shape_selector(&shape).map_err(|e| {
        invalid_request_error("Invalid shape selector", e.to_string(), Some(json!({"operation":"shape_info_impl","phase":"parse_shape"})))
    })?;
    let formatter = shex_ast::compact::ShExFormatter::default().without_colors();
    serialize_shape_current_shex_rudof(&rudof, &shape_selector, &parsed_result_format, &formatter, &mut output_buffer)
        .map_err(|e| internal_error("Serialization failed", e.to_string(), Some(json!({"operation":"shape_info_impl","phase":"serialize_shape"}))))?;

    let mut dependencies_included = false;
    let mut statistics_included = false;
    if compile.unwrap_or(false) && rudof.get_shex_ir().is_some() {
        if show_dependencies.unwrap_or(false) { dependencies_included = true; }
        if show_statistics.unwrap_or(false) { statistics_included = true; }
    }

    let output_bytes = output_buffer.into_inner();
    let output_str = String::from_utf8(output_bytes).map_err(|e| {
        internal_error("Conversion error", e.to_string(), Some(json!({"operation":"shape_info_impl","phase":"utf8_conversion"})))
    })?;
    ...
    let response = ShapeInfoResponse {
        ...
        result_size_bytes,
        result_lines,
        elapsed_seconds: if show_time.unwrap_or(false) { Some(begin.elapsed().as_secs_f64()) } else { None },
        dependencies_included: if dependencies_included { Some(true) } else { None },
        statistics_included: if statistics_included { Some(true) } else { None },
    };

    let structured = serde_json::to_value(&response).map_err(|e| {
        internal_error("Serialization error", e.to_string(), Some(json!({"operation":"shape_info_impl","phase":"serialize_response"})))
    })?;
...
pub async fn convert_shex_impl(
    service: &RudofMcpService,
    Parameters(ConvertShexRequest { schema, schema_format, base_schema, reader_mode, result_schema_format, show_time }): Parameters<ConvertShexRequest>,
) -> Result<CallToolResult, McpError> {
    let parsed_schema_format: ShExFormat = match schema_format {
        Some(s) => ShExFormat::from_str(&s).map_err(|e| {
            invalid_request_error("Invalid schema format", e.to_string(),
    ...
    let schema_spec = InputSpec::Str(schema.clone());
    parse_shex_schema(&mut rudof, &schema_spec, &parsed_schema_format, &parsed_base_schema, &parsed_reader_mode, &rudof_config)
        .map_err(|e| internal_error("Parsing ShEx schema failed", e.to_string(), Some(json!({"operation":"convert_shex_impl","phase":"parse_shex"}))))?;

    // Serialize requested output into buffer (whole schema)
    let formatter = shex_ast::compact::ShExFormatter::default().without_colors();
    serialize_current_shex_rudof(&rudof, &parsed_result_format, &formatter, &mut output_buffer)
        .map_err(|e| internal_error("Serialization failed", e.to_string(), Some(json!({"operation":"convert_shex_impl","phase":"serialize_schema"}))))?;

    let output_bytes = output_buffer.into_inner();
    let output_str = String::from_utf8(output_bytes).map_err(|e| {
        internal_error("Conversion error", e.to_string(), Some(json!({"operation":"convert_shex_impl","phase":"utf8_conversion"})))
    })?;
    ...
    let structured = serde_json::to_value(&response).map_err(|e| {
        internal_error("Serialization error", e.to_string(), Some(json!({"operation":"convert_shex_impl","phase":"serialize_response"})))
    })?;
