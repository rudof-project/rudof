...
fn discover_cases(root: &Path) -> Vec<Case> {
    STEP_SIZES
        ...
        .map(|&n| Case {
            n,
            schema: root.join(format!("{n}_steps")).join(format!("shape_{n}_steps_ShEx.shex")),
            data: root.join(format!("{n}_steps")).join(format!("data_graph_{n}_steps_ShEx.ttl")),
            shapemap: root.join(format!("{n}_steps")).join(format!("shape_map_{n}_steps_entity_0.shex")),
        })
