...
impl FieldGenerator for IntegerGenerator {
    fn generate(&self, context: &GenerationContext) -> Result<String> {
        ...
        let min = context.parameters.get("min")
            .and_then(|v| v.as_i64())
            .unwrap_or(-1000) as i32;
            
        let max = context.parameters.get("max")
            .and_then(|v| v.as_i64())
...
impl FieldGenerator for DecimalGenerator {
    fn generate(&self, context: &GenerationContext) -> Result<String> {
        ...
        let precision = context.parameters.get("precision")
            .and_then(|v| v.as_u64())
...
impl FieldGenerator for BooleanGenerator {
    fn generate(&self, context: &GenerationContext) -> Result<String> {
        ...
        let true_probability = context.parameters.get("true_probability")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.5);
...
impl FieldGenerator for UriGenerator {
    fn generate(&self, _context: &GenerationContext) -> Result<String> {
        ...
        let domains = ["example.com", "example.org", "example.net", "company.xyz", "research.edu"];
...
fn generate_name(rng: &mut impl Rng, locale: &str) -> String {
    let first_names = match locale {
        "es" => &["Ana", "Carlos", "María", "Diego", "Elena", "Fernando", "Isabel", "Jorge", "Laura", "Miguel"],
        "fr" => &["Alice", "Bernard", "Claire", "David", "Élise", "François", "Gabrielle", "Henri", "Isabelle", "Jacques"],
        _ => &["Alice", "Bob", "Charlie", "Diana", "Edward", "Fiona", "George", "Hannah", "Ian", "Julia"],
    };
    
    let last_names = match locale {
        "es" => &["García", "Rodríguez", "González", "Fernández", "López", "Martínez", "Sánchez", "Pérez", "Gómez", "Ruiz"],
        "fr" => &["Martin", "Bernard", "Dubois", "Thomas", "Robert", "Richard", "Petit", "Durand", "Leroy", "Moreau"],
        _ => &["Smith", "Johnson", "Williams", "Brown", "Jones", "Garcia", "Miller", "Davis", "Rodriguez", "Martinez"],
    };
...
fn generate_title(rng: &mut impl Rng, locale: &str) -> String {
    let adjectives = match locale {
        "es" => &["Moderno", "Avanzado", "Eficiente", "Innovador", "Práctico", "Teórico", "Aplicado"],
        "fr" => &["Moderne", "Avancé", "Efficace", "Innovant", "Pratique", "Théorique", "Appliqué"],
        _ => &["Modern", "Advanced", "Efficient", "Innovative", "Practical", "Theoretical", "Applied"],
    };
    
    let nouns = match locale {
        "es" => &["Sistema", "Proyecto", "Análisis", "Investigación", "Desarrollo", "Plataforma"],
        "fr" => &["Système", "Projet", "Analyse", "Recherche", "Développement", "Plateforme"],
...
fn generate_email(rng: &mut impl Rng) -> String {
    let prefixes = ["user", "admin", "contact", "info", "support", "sales", "john.doe", "jane.smith"];
