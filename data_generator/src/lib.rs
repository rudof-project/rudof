...
impl DataGenerator {
    ...
    pub async fn run_with_format<P: AsRef<Path>>(&mut self, schema_path: P, format: Option<SchemaFormat>) -> Result<()> {
    ...
    fn convert_unified_to_shape_infos(&self, unified_model: &crate::unified_constraints::UnifiedConstraintModel) -> Vec<crate::shape_processing::ShapeInfo> {
        use crate::shape_processing::{ShapeInfo, ShapeDependency, PropertyInfo};
        ...
        for (shape_id, unified_shape) in &unified_model.shapes {
            ...
            for prop in &unified_shape.properties {
                ...
                let datatype = prop.constraints.iter()
                    .find_map(|c| match c {
                ...
                let shape_ref = prop.constraints.iter()
                    .find_map(|c| match c {
