...
mod tests {
    ...
    fn cnv(se: &SE) -> Result<SE1, SErr> {
        match se {
          SE::And { es} => {
            let vs: Vec<Result<SE1, SErr>> = es.iter().map(|se| {
               let r = cnv(se);
               r
            }).collect();
          ...
          SE::S { v } => {
            match v.parse::<i32>() {
                Ok(n) => Ok(SE1::S { v: n }),
                Err(e) => Err( 
                    SErr::Cnv { msg: format!("Error converting {v} to i32: {e}")}
                )
            }
          }
        }
    ...
    fn test_se_conversion() {
        let se = SE::And { es: vec![SE::Not { e: Box::new(SE::S { v: "23".to_string()})}, SE::S { v: "43".to_string()}] }; 
        let expected = SE1::And { es: vec![SE1::Not { e: Box::new(SE1::S { v: 23 })}, SE1::S { v: 43 }] }; 
        assert_eq!(cnv(&se), Ok(expected))
    }

    #[test]
    fn test_se_conversion_err() {
        let se = SE::And { es: vec![SE::Not { e: Box::new(SE::S { v: "foo".to_string()})}, SE::S { v: "43".to_string()}] }; 
