...
impl<R: Rdf> NativeValidator<R> for Class<R> {
    fn validate_native(
        &self,
        ...
        let class = |value_node: &R::Term| {
            if R::term_is_literal(value_node) {
                return true;
            }

            let is_class_valid =
                get_objects_for(store.inner_store(), value_node, &R::iri_s2iri(&RDF_TYPE))
                    .unwrap_or_default()
                    .iter()
                    .any(|ctype| {
...
impl<R: Sparql> SparqlValidator<R> for Class<R> {
    fn validate_sparql(
        &self,
        component: &CompiledComponent<R>,
        shape: &CompiledShape<R>,
        ...
        let query = move |value_node: &R::Term| {
            formatdoc! {"
