...
impl ValidationResult {
    pub(crate) fn parse<S: FocusRDF>(store: &mut S, validation_result: &S::Term) -> Result<Self, ResultError> {
        ...
        let constraint_component = match store
            .object_for(
                validation_result,
                &ShaclVocab::sh_source_constraint_component().into(),
            )
            .map_err(|e| ResultError::ObjectFor {
                subject: validation_result.to_string(),
        ...
        let sh_result_path_iri: S::IRI = ShaclVocab::sh_result_path().into();
        let path = store
        ...
        let sh_source_shape_iri: S::IRI = ShaclVocab::sh_source_shape().into();
        let source = store
        ...
        let sh_value_iri: S::IRI = ShaclVocab::sh_value().into();
        let value = store
        ...
        RDF: BuildRDF + Sized,
    {
        ...
        rdf_writer
            .add_triple(report_node.clone(), ShaclVocab::sh_result_message(), message)
            .map_err(|e| ReportError::ValidationError {
        ...
        if let Some(source) = &self.source {
            ...
            rdf_writer
                .add_triple(report_node.clone(), ShaclVocab::sh_source_shape(), source_term)
                .map_err(|e| ReportError::ValidationError {
        ...
        if let Some(value) = &self.value {
            ...
            rdf_writer
                .add_triple(report_node.clone(), ShaclVocab::sh_value(), value_term)
                .map_err(|e| ReportError::ValidationError {
