...
impl<R: Rdf> ValidationResult<R> {
    ...
    pub(crate) fn parse(store: &R, validation_result: &Object<R>) -> Result<Self, ResultError> {
        ...
        let focus_node = match get_object_for(
            store,
            validation_result,
            &Predicate::<R>::from_str(SH_FOCUS_NODE.as_str()),
        )? {
        ...
        let severity = match get_object_for(
            store,
            validation_result,
            &Predicate::<R>::from_str(SH_RESULT_SEVERITY.as_str()),
        )? {
        ...
        let constraint_component = match get_object_for(
            store,
            validation_result,
            &Predicate::<R>::from_str(SH_SOURCE_CONSTRAINT_COMPONENT.as_str()),
        )? {
        ...
        let path = get_object_for(
            store,
            validation_result,
            &Predicate::<R>::from_str(SH_RESULT_PATH.as_str()),
        )?;
        let source = get_object_for(
            store,
            validation_result,
            &Predicate::<R>::from_str(SH_SOURCE_SHAPE.as_str()),
        )?;
        let value = get_object_for(
            store,
            validation_result,
            &Predicate::<R>::from_str(SH_VALUE.as_str()),
        )?;
