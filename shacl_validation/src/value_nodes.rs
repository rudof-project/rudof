...
pub struct ValueNodes<R: Rdf>(HashMap<Object<R>, FocusNodes<R>>);

impl<R: Rdf> ValueNodes<R> {
    pub fn new(iter: impl Iterator<Item = (Object<R>, FocusNodes<R>)>) -> Self {
...
pub trait IterationStrategy<R: Rdf> {
    ...
    ) -> Box<dyn Iterator<Item = (&'a Object<R>, &'a Self::Item)> + 'a>;
}
