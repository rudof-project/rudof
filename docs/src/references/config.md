...
| `recursion_semantics` | `"none"` \| `"cautious"` \| `"brave"` | `"cautious"` | Whether a shape that (directly or transitively) references itself is accepted at all, and if so, how a cycle in the data is resolved. `none` rejects the shapes graph outright — a schema-loading error — the moment it contains such a cycle, regardless of the data. `cautious` (the default) accepts it and assumes a node caught in a cycle does **not** conform unless that can be shown without relying on the cycle; `brave` accepts it and assumes it **does** conform as long as that assumption is self-consistent. A cycle built only from monotonic constructs (`sh:and`, `sh:or`, `sh:node`, `sh:property`, `sh:minCount`, `sh:closed`, ...) is always sound under `cautious`/`brave`; one that also carries a negating constraint (`sh:not`, `sh:xone`, ...) is sound too as long as it's *stratified* — every such constraint targets a shape that doesn't itself depend on any recursion. A cycle where negation instead reaches back into a recursive shape has no safe order to resolve it in and is always rejected, regardless of this setting — see [Recursion and negation](../cli_usage/shacl_validate.md#recursion-and-negation). |
