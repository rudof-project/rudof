...
`rudof` has some basic support for SPARQL queries, run with the `query` command. It is possible not only to perform SPARQL queries on SPARQL endpoints, but also on local files. To load and inspect a query without running it, see [`sparql`](./sparql.md).
...
See the [Backend (`--backend`) reference](./backend.md) for full documentation.

## Querying a property graph database with Cypher

`query` also accepts Cypher queries against a connected LadybugDB database via `--dialect cypher` — see [Querying with Cypher](./connect.md#querying-with-cypher).

## Related commands

- [`rudof sparql`](./sparql.md): load and show a SPARQL query without running it
