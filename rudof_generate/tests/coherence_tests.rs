...
"#);
    ...
    assert_eq!(property_triples, 100, "Should have exactly 100 properties (5 per instance)");
    ...
    for i in 1..=10 {
        ...
        assert!(output_content.contains(&prop), "Property {} should appear at least once in 20 entities", prop);
    }
}
...
"#);
    ...
    for line in output_content.lines() {
        ...
        if line.contains("http://www.w3.org/1999/02/22-rdf-syntax-ns#type") { continue; }
    ...
    assert!(max_count - min_count >= 3, "Should have variance in property counts. Range got: {}-{}", min_count, max_count);
...
"#;
    ...
    assert!(output_content.contains("http://example.org/keep"), "Should contain non-excluded property");
    assert!(!output_content.contains("http://example.org/skip"), "Should NOT contain excluded property");
}
...
"#;
    ...
    overrides.insert("http://example.org/ShapeLow".to_string(), TypeOverrideConfig {
